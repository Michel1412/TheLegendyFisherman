# TLF Vision Refactor

> Fonte da verdade para a visao de produto, arquitetura e progressao do modpack **The Legendary Fishman**.

---

## Como usar este documento

Preencha este arquivo antes de iniciar refatoracoes de sistemas, quests, economia, progressao ou curadoria de mods.

- Use `TBD` para decisoes ainda abertas.
- Registre decisoes firmes como texto direto, nao como brainstorm.
- Quando uma secao mudar uma decisao tecnica, atualize tambem os documentos relacionados em `.cursor/internal-docs/` ou `.cursor/architecture/`.

## Documentos derivados

- Indice geral: [`../README.md`](../README.md)
- Identidade: [`identity.md`](./identity.md)
- Arquitetura: [`../architecture/architecture.md`](../architecture/architecture.md)
- Gameplay loop: [`../gameplay/gameplay-loop.md`](../gameplay/gameplay-loop.md)
- Progressao: [`../gameplay/progression.md`](../gameplay/progression.md)
- Rotas: [`../gameplay/routes.md`](../gameplay/routes.md)

---

## Filosofia central

> "Viva como um pescador humilde, aproveitando a paz e da leveza da vida"

Esta e a promessa emocional do TLF. O jogador comeca pequeno: uma vara ruim, uma cabana simples, chuva na praia e pouco controle sobre o oceano. A progressao nao deve transformar essa fantasia em pressa industrial; ela deve ampliar a vida costeira com cuidado, comunidade e dominio artesanal.

- Pescador reforca paciencia, tecnica e respeito ao mar.
- Cozinheiro transforma capturas simples em conforto e preparo.
- Artesao faz a vida no cais parecer construida pelas proprias maos.
- Marinheiro amplia a costa sem perder o sentimento de casa.
- MineColonies representa a virada de abrigo humilde para porto vivo.

---

## Validacao de Mods

### Objetivo

TBD

### Criterios de permanencia

- TBD

### Criterios de remocao

- TBD

### Fonte auxiliar

Auditoria de mods: [`../architecture/mods-audit.md`](../architecture/mods-audit.md)

---

## Riscos Tecnicos

### Riscos conhecidos

- TBD

### Riscos aceitos

- TBD

### Riscos que bloqueiam refatoracao

- TBD

---

## Conflitos Conceituais

### Conflitos entre mods

- TBD

### Conflitos entre mecanicas

- TBD

### Conflitos com a fantasia central

- TBD

---

## Arquitetura Ideal

### Principios

- TBD

### Sistemas centrais

- TBD

### Sistemas de suporte

- TBD

### Limites de escopo

- TBD

---

## Identidade

### Promessa do modpack

TBD

### Tom e atmosfera

TBD

### Papel do jogador

TBD

---

## Expansao Fantasia

### Fantasia maritima

TBD

### Fantasia de vila e comunidade

TBD

### Fantasia de profissao

TBD

---

## Gameplay Loop

### Loop inicial

TBD

### Loop intermediario

TBD

### Loop avancado

TBD

---

## Mecanicas

### Pesca

TBD

### Culinaria

TBD

### Artesanato

TBD

### Navegacao

TBD

### Vila, colonos e reputacao

TBD

---

## Progressao

### Tiers

TBD

### Insignias

TBD

### Desbloqueios

TBD

### Requisitos

TBD

---

## Endgame

### Objetivo final

TBD

### Conteudo repetivel

TBD

### Recompensas finais

TBD

---

## Especializacoes

### Pescador

TBD

### Cozinheiro

TBD

### Artesao

TBD

### Marinheiro

TBD

---

## Reputacao

### Fontes de reputacao

TBD

### Usos de reputacao

TBD

### Reputacao local vs global

TBD

---

## Rotas Oceanicas

### Tipos de rota

TBD

### Progressao por rota

TBD

### Recompensas por exploracao

TBD

---

## Lendas do Mar

### Definicao

TBD

### Como encontrar

TBD

### Como vencer ou completar

TBD

### Recompensas

TBD

---

## Pacing e Economia

### Ritmo esperado

TBD

### Economia singleplayer

TBD

### Economia multiplayer

TBD

### Sinks e fontes de recursos

TBD

---

## Generos

### RPG

TBD

### Sandbox

TBD

### Cozy

TBD

### Fishing Adventure

TBD

---

## Decisoes Abertas

- TBD

## Decisoes Firmadas

- TBD
